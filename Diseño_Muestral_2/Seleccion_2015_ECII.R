...
Marcoest_saber2015_Aquia <- readRDS("saber2015_Aquia.rds")

length(unique(Marcoest_saber2015_Aquia$COLE_COD_MCPIO_UBICACION))
length(unique(Marcoest_saber2015_Aquia$COLE_COD_DANE_INSTITUCION))
length(unique(Marcoest_saber2015_Aquia$ESTU_AREA_RESIDE ))

cole <- Marcoest_saber2015_Aquia %>% group_by(COLE_COD_ICFES) %>% summarise(tx = n())
sum(cole$tx)
...
marco_cole <- merge(Marcoest_saber2015_Aquia, mue_cole, all.y = T, by = "COLE_COD_ICFES") 
...
EC2muestraXest2015 <- muestra[indica_mueestu$ID_unit,]  

EC2muestraXest2015 <- merge(EC2muestraXest2015,  consulta_estud)
length(unique(EC2muestraXest2015$COLE_COD_ICFES))
length(unique(EC2muestraXest2015$COLE_COD_MCPIO_UBICACION))

setwd("D:/Documents/MEA/SAE/Estudio_Caso_II/Modelo_Area_Temporal_Yu_Rao_Saber_Antioquia/Muestra_2")
saveRDS(EC2muestraXest2015, "muestra2Etapas_2015.RDS")

library(survey)
# Diseño Muestral 
names(EC2muestraXest2015)
diseno_muestral <- svydesign(ids = ~ COLE_COD_ICFES + ESTU_CONSECUTIVO,
                             strata = ~ estrato_cole,
                             fpc = ~ Nh + N_i, data = EC2muestraXest2015,
                             nest = T)

svymean(~PUNT_GLOBAL, diseno_muestral)
100 * cv(svymean(~PUNT_GLOBAL, diseno_muestral))

max(EC2muestraXest2015$PUNT_GLOBAL)
mean(EC2muestraXest2015$PUNT_GLOBAL)
mean(Marcoest_saber2015_Aquia$PUNT_GLOBAL)           
max(Marcoest_saber2015_Aquia$PUNT_GLOBAL)
